...
public class StudentCtl extends BaseCtl {
	...
	protected BaseDTO populateDTO(HttpServletRequest request) {
		...
		long id = (DataUtility.getLong(request.getParameter("collegeId")));
		if (id>0) {
		System.out.println("Heloooooo + id : "+ id)	;
		try {
			cdto = cmodel.findByPK(id);

		} catch (ApplicationException e) {
			e.printStackTrace();
		}
		dto.setCollegeName(DataUtility.getString(cdto.getName()));
		}
